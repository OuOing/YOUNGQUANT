...
const ChatTerminal = ({ symbol, price, period, stockName }) => {
  ...
  const fetchNewsAndGreet = async (isInitial) => {
    if (isInitial) setMessages([{ role: 'ai', text: '👋 正在为您同步全球市场情报...', id: Date.now() }]);
    try {
      const res = await fetch('/api/news');
      const news = await res.json();
      let text = "👋 你好！我是 YoungQuant-v1。今日财经核心内参：\n\n";
      ...
      <div className="mt-8 pt-8 border-t border-white/5">
        ...
        <div className="relative group/input">
          <input 
            ...
            className="w-full bg-black/60 border-2 border-white/10 rounded-2xl p-6 text-base outline-none focus:border-white/30 transition-all pr-20 text-white font-bold hover:border-white/20 shadow-inner"
          />
          <button 
            onClick={() => handleSend()}
